...
class GUI(Frame):
    ...
    def ask_canvas_size(self):
        ...
        self.canvas_width_label.pack(side=LEFT, padx=(10,0))
        ...
        self.canvas_height_label.pack(side=LEFT, padx=(10,0))
        ...
        self.canvas_size_ask_button.pack(side=LEFT,padx=10)
        self.ask_info_bar = Label(self.frame_ask,text="")
        self.ask_info_bar.pack(side=LEFT)

        self.frame_ask.pack(pady=10,padx=10)
        self.pack()
    def check_init(self):
        try:
            if self.canvas_name_entry.get().strip() == "" or int(self.canvas_width_entry.get())<= 100 or int(self.canvas_height_entry.get())<=100:
            ...
            self.ask_info_bar.configure(text="Please check the information and try again.")
    def initUI(self, name="gui", w=900, h=500):
        self.name = name
        ...
        self.color_4 = "gray30"
        self.listbox_bgcolor = "gray20"
        self.color_toolbar_options_bg = "gray10"
        self.color_toolbar_bg = "gray10"
        self.color_radio_on = "gray20"
        self.color_radio_off = "gray10"
        ...
        self.dict_label = {}
        self.dict_list = {}
        self.dict_layer_boolean = {}
        self.dict_icons_to_save = {}
        self.export_content = {}
        self.dict_layer_boolean["Background"] = False
        ...
        self.button_export.pack(fill=Y, ipadx=10, side=LEFT)
        #---

        # TOOLBAR TOOL OPTIONS
        self.all_options = []
        self.frame_tools_option = Frame(self, bg=self.color_toolbar_options_bg)
        self.invisible_placeholder = Frame(self.frame_tools_option,height=0,bg=self.color_main_bg)
        self.all_options.append(self.invisible_placeholder)
        self.invisible_placeholder.pack()
        #---
        ...
        self.label_button_text = Label(self.frame_tools_option_button, text="Button Text: ", fg="white", bg=self.color_toolbar_options_bg)
        self.label_button_text.pack(side=LEFT)
        self.entry_button_text = Entry(self.frame_tools_option_button, width=30)
        self.entry_button_text.pack(side=LEFT)

        self.label_button_relief = Label(self.frame_tools_option_button, text="Button Relief: ", fg="white", bg=self.color_toolbar_options_bg)
        self.label_button_relief.pack(side=LEFT, padx=(30,0))
        self.radio_button_groove = Radiobutton(self.frame_tools_option_button, text="Groove", bg=self.color_toolbar_options_bg, fg="white", selectcolor=self.color_toolbar_options_bg, variable=self.relief_var, value="groove")
        self.radio_button_groove.pack(side=LEFT)
        self.radio_button_flat = Radiobutton(self.frame_tools_option_button, text="Flat", bg=self.color_toolbar_options_bg, fg="white", selectcolor=self.color_toolbar_options_bg, variable=self.relief_var, value="flat")
        self.radio_button_flat.pack(side=LEFT)
        self.radio_button_raised = Radiobutton(self.frame_tools_option_button, text="Raised", bg=self.color_toolbar_options_bg, fg="white", selectcolor=self.color_toolbar_options_bg, variable=self.relief_var, value="raised")
        self.radio_button_raised.pack(side=LEFT)
        self.radio_button_ridge = Radiobutton(self.frame_tools_option_button, text="Ridge", bg=self.color_toolbar_options_bg,fg="white", selectcolor=self.color_toolbar_options_bg, variable=self.relief_var,value="ridge")
        self.radio_button_ridge.pack(side=LEFT)
        ...
        self.combo_button_font = Combobox(self.frame_tools_option_button, state="readonly",values=sorted(tkFont.families()), width=20)
        ...
        self.color_button_fg.pack(side=LEFT, padx=(10,0))
        ...
        self.combo_text_font = Combobox(self.frame_tools_option_text, state="readonly", values=sorted(tkFont.families()),width=20)
        ...
        self.color_text_fg.pack(side=LEFT, padx=(10,0))
        ...
        #---
        self.frame_tools_option.pack(fill=X, pady=(0,10),ipadx=10)
        ...
        self.radio_move = Radiobutton(self.frame_tools,height=20,width=20,  image=self.image_move, activebackground=self.color_radio_active, selectcolor=self.color_radio_on, bg=self.color_radio_off, offrelief="flat", indicatoron=False, variable=self.tool_var, value="Move", command=self.show_tool_options)
        self.radio_move.pack(pady=(8,5))
        self.radio_draw_button = Radiobutton(self.frame_tools,height=20,width=20, image=self.image_add_button, activebackground=self.color_radio_active, selectcolor=self.color_radio_on, bg=self.color_radio_off, offrelief="flat", indicatoron=False, variable=self.tool_var, value="Create Button", command=self.show_tool_options)
        self.radio_draw_button.pack(pady=5)
        self.radio_draw_list = Radiobutton(self.frame_tools,height=20,width=20, image=self.image_add_list, activebackground=self.color_radio_active, selectcolor=self.color_radio_on, bg=self.color_radio_off, offrelief="flat", indicatoron=False, variable=self.tool_var, value="Create List", command=self.show_tool_options)
        self.radio_draw_list.pack(pady=5)
        self.radio_draw_text = Radiobutton(self.frame_tools,height=20,width=20, image=self.image_add_text, activebackground=self.color_radio_active, selectcolor=self.color_radio_on, bg=self.color_radio_off, offrelief="flat", indicatoron=False, variable=self.tool_var, value="Create Label", command=self.show_tool_options)
        ...
        self.frame_canvas.pack(side=LEFT, padx=10, pady=(0,20), fill=BOTH, expand=True)

    #RIGHT PART
        self.frame_right = Frame(self, bg=self.color_main_bg)
        self.frame_right.pack(padx=(0,10), anchor="ne")
        # LAYERS
        ...
        self.layers = Listbox(self.frame_layer, bd=0,highlightthickness=0, width=49,fg="white", relief="flat", bg=self.listbox_bgcolor, selectbackground=self.color_lb_selected,highlightcolor=self.color_main_bg, highlightbackground = self.color_main_bg)
        ...
        self.options_frame_bg.pack(side=LEFT, padx=(0,20))
        ...
        self.frame_icon = Frame(self.frame_right)
        self.all_options.append(self.frame_icon)
        self.current_page = 1
        self.frame_icon_page = Frame(self.frame_icon)
        self.icon_page_left = Button(self.frame_icon, bg=self.color_1, bd=0, activebackground=self.color_2, activeforeground="white", relief="flat", fg="white",text=u"\u2BC7",command=self.go_left)
        self.icon_page_left.pack(fill=Y,side=LEFT)
        self.frame_icon_page.pack(side=LEFT)
        self.icon_page_right = Button(self.frame_icon, bg=self.color_1, bd=0, activebackground=self.color_2, activeforeground="white", relief="flat", fg="white",text=u"\u2BC8",command=self.go_right)
        self.icon_page_right.pack(fill=Y,side=LEFT)
        self.iconvar = IntVar()
        self.iconvar.set(0)
        self.icon_imagedb = {}
        self.icon_radiodb = {}
    ...
    def go_page(self, page):
        row = 0
        column = 0
        for i in range((page-1)*140, page*140):
            self.imagex = base64.b64decode(encoded_icon_list.icon_list[i])
            self.buf = io.BytesIO(self.imagex)
            self.img = PImage.open(self.buf)
            self.icon_imagedb[i] = ImageTk.PhotoImage(self.img)
            self.icon_radiodb[i] = Radiobutton(self.frame_icon_page, bg=self.listbox_bgcolor, relief="flat", bd=0, image = self.icon_imagedb[i], variable=self.iconvar, indicatoron=False, value=i, command=self.add_icon_to_button)
            if (i+1)%14 != 0:
                self.icon_radiodb[i].grid(row=row, column=column)
                column+=1
            else:
                row+=1
                column=0
    ...
    def add_icon_to_button(self):
        a = self.iconvar.get()
        self.dict_button[self.selected_layer].configure(image=self.icon_radiodb[a].cget("image"), compound="left")
        self.dict_icons_to_save[self.selected_layer] = a


    def entered(self, event=None):
        if self.tool_var.get() == "Create Button" or self.tool_var.get() == "Create List" or self.tool_var.get() == "Create Label" :
            if self.dict_layer_boolean[self.selected_layer] == True:
                self.main_canvas.configure(cursor="cross")
            else:
                self.main_canvas.configure(cursor="X_cursor")
        else:
            self.main_canvas.configure(cursor="cross")
        if self.selected_layer == "Background":
            self.main_canvas.configure(cursor="X_cursor")
    ...
    def show_tool_options(self):
        ...
        if self.tool_var.get() == "Create Button":
            ...
            self.frame_icon.pack(anchor=CENTER,pady=(0,20))
        if self.tool_var.get() == "Create Label":
            self.frame_tools_option_text.pack(padx=4,anchor=W,fill=X,expand=True)



    def add_layer(self,event=None,name="",):

        self.layers.insert(0, tkSimpleDialog.askstring("New layer", "Layer Name").replace(" ", "_"))
        self.layer_number += 1
        self.layers.selection_clear(0,END)
        self.layers.select_set(0)
        self.select_layer()
        self.dict_layer_boolean[self.layers.get(self.layers.curselection())] = True


    def del_layer(self, event=None):
        if self.layers.size()>0 and self.selected_layer != "Background":
            result = tkMessageBox.askyesno("Delete Layer", "Would you like to delete "+self.selected_layer+"?")
            if result:
                try:
                    del self.dict_button[self.selected_layer]
                except KeyError:
                    pass
                try:
                    del self.dict_list[self.selected_layer]
                except KeyError:
                    pass
                try:
                    del self.dict_label[self.selected_layer]
                except KeyError:
                    pass
    ...
    def on_click(self, event):
        if self.selected_layer != "" and self.selected_layer != "Background":
            if self.tool_var.get() == "Create Button":
                if self.dict_layer_boolean[self.selected_layer] == True:
                    self.x = event.x
                    self.y = event.y
                    ...
                    self.dict_button[self.selected_layer] = Button(self.button_frame,text=self.entry_button_text.get(),   font=(self.combo_button_font.get(), self.combo_button_font_size.get()),fg=self.color_button_fg.cget("bg"), bg=self.color_button_bg.cget("bg"), relief=self.relief_var.get())
                    if event.x % 20 <= 10:
                        self.placex = event.x - event.x % 20
                    else:
                        self.placex = event.x + 20 - event.x % 20
                    if event.y % 20 <= 10:
                        self.placey = event.y - event.y % 20
                    else:
                        self.placey = event.y + 20 - event.y % 20
                    self.button_frame_window = self.main_canvas.create_window(self.placex, self.placey, window=self.button_frame, anchor=N+W, tags=self.selected_layer)
                    self.dict_button[self.selected_layer].pack(fill=BOTH, ipady=5, expand=True)
                ...
                if self.dict_layer_boolean[self.selected_layer] == True:
                    ...
                    self.dict_list[self.selected_layer] = [Listbox(self.list_frame, relief="flat")]
                    self.dict_list[self.selected_layer].append(CustomScrollbar(self.list_frame, self.dict_list[self.selected_layer][0], command=self.dict_list[self.selected_layer][0].yview))
                    if event.x % 20 <= 10:
                        self.placex = event.x - event.x % 20
                    else:
                        self.placex = event.x + 20 - event.x % 20
                    if event.y % 20 <= 10:
                        ...
                        self.placey = event.y + 20 - event.y % 20
                    self.list_frame_window = self.main_canvas.create_window(self.placex, self.placey, window=self.list_frame, anchor=N+W, tags=self.selected_layer)
                    self.dict_list[self.selected_layer][0].pack(fill=BOTH, expand=True, side=LEFT)
                    self.dict_list[self.selected_layer][1].pack(fill=Y,expand=True)
                ...
                if self.dict_layer_boolean[self.selected_layer] == True:
                    ...
                    self.dict_label[self.selected_layer] = Label(self.text_frame, text=self.entry_text_text.get(), bg=self.color_text_bg.cget("bg"), fg=self.color_text_fg.cget("bg"),font=(self.combo_text_font.get(), self.combo_text_font_size.get()))
                    if event.x % 20 <= 10:
                        self.placex = event.x - event.x % 20
                    else:
                        self.placex = event.x + 20 - event.x % 20
                    if event.y % 20 <= 10:
                        ...
                        self.placey = event.y + 20 - event.y % 20
                    self.text_frame_window = self.main_canvas.create_window(self.placex, self.placey, window=self.text_frame, anchor=N+W, tags=self.selected_layer)
                    self.dict_label[self.selected_layer].pack(fill=BOTH, expand=True)
            ...
            if self.tool_var.get() == "Create Button":
                if self.dict_layer_boolean[self.selected_layer] == True:
                    if event.x <= self.main_canvas.winfo_reqwidth() and event.y <= self.main_canvas.winfo_reqheight():
                        if event.x % 20 <= 10:
                            width = (event.x - event.x % 20) - self.placex
                        else:
                            width = (event.x + 20 - event.x % 20)  - self.placex
                        if event.y % 20 <= 10:
                            height = (event.y - event.y % 20)  - self.placey
                        else:
                            height = (event.y + 20 - event.y % 20)  - self.placey

                        self.main_canvas.itemconfig(self.button_frame_window,width=width,height=height)
            elif self.tool_var.get() == "Create List":
                if self.dict_layer_boolean[self.selected_layer] == True:
                    if event.x <= self.main_canvas.winfo_reqwidth() and event.y <= self.main_canvas.winfo_reqheight():
                        if event.x % 20 <= 10:
                        ...
                        self.main_canvas.itemconfig(self.list_frame_window,width=width,height=height)
                        self.dict_list[self.selected_layer][1].height = height
                ...
                if self.dict_layer_boolean[self.selected_layer] == True:
                    if event.x <= self.main_canvas.winfo_reqwidth() and event.y <= self.main_canvas.winfo_reqheight():
                        ...
                        self.main_canvas.itemconfig(self.text_frame_window,width=width,height=height)


            elif self.tool_var.get() == "Draw":
                # self.main_canvas.create_line(self.x, self.y, event.x, event.y,width=1.2, fill=self.colors,tags=self.selected_layer)
                self.main_canvas.create_line(self.x, self.y, event.x,event.y, smooth=1, fill=self.color,tags=self.selected_layer)
                self.x = event.x
                self.y = event.y
            elif self.tool_var.get() == "Move":
                ...
                dy = event.y - self.y
                if dx>= 10:
                    self.main_canvas.move(self.selected_layer, 10, 0)
                    self.x = event.x
                if dx <=-10:
                    self.main_canvas.move(self.selected_layer, -10, 0)
                    self.x = event.x
                if dy >= 10:
                    self.main_canvas.move(self.selected_layer, 0,10)
                    self.y = event.y
                if dy <= -10:
                    self.main_canvas.move(self.selected_layer, 0,-10)
                    self.y = event.y
    ...
    def off_click(self, event):
        if self.selected_layer == "":
            pass
        elif self.tool_var.get()=="Create Button" or self.tool_var.get()=="Create List" or self.tool_var.get()=="Create Label":
            self.dict_layer_boolean[self.selected_layer] = False
...
lines = file.readlines()\n""" % (self.name,self.icon, width, height)
        content =""
        for i in sorted(self.dict_label):
            layer = i
            if layer != "Background":
                widget_x = self.main_canvas.coords(layer)[0]
                widget_y = self.main_canvas.coords(layer)[1]
                bg = self.dict_label[layer].cget("bg")
                fg = self.dict_label[layer].cget("fg")

                text = self.dict_label[layer].cget("text")
                font = self.dict_label[layer].cget("font")
                ...
                content += "%s = Label(frame_%s, text='%s', bg='%s', fg='%s', font='%s')\n" % (str(layer),str(layer), text, bg, fg, font)
...
icondb["%s"] = ImageTk.PhotoImage(img)\n""" % (self.dict_icons_to_save[i],i)
                widget_width = "width="+str(self.main_canvas.itemcget(layer,"width"))
                widget_height = "height="+str(self.main_canvas.itemcget(layer,"height"))
                content += "\t#%s\n" % layer
                content += "frame_%s = Frame(root,%s,%s)\n" % (str(layer),widget_height,widget_width)
                content += "frame_%s.pack_propagate(0)\n"%(str(layer))
                content += "%s = Button(frame_%s, compound='left', text='%s', bg='%s', fg='%s', relief='%s', font='%s',image=icondb['%s'])\n" % (str(layer),str(layer), text, bg, fg, relief, font,i)
                content += "%s.pack(fill=BOTH,expand=True)\n" % layer
                content += "frame_%s.place(x=%s,y=%s,anchor=N+W)\n" % (str(layer), widget_x, widget_y)
                content += "\t#---\n\n"
